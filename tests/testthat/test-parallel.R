...
test_that("TMB tape construction is sequential unless explicitly enabled", {
  previous <- TMB::config(DLL = "rpbnb")$tape.parallel
  on.exit(
    TMB::config(tape.parallel = previous, DLL = "rpbnb"),
    add = TRUE
  )
  ...
  expect_identical(TMB::config(DLL = "rpbnb")$tape.parallel, 0L)
...
test_that("TMB thread configuration realizes a valid request", {
  ...
  expect_identical(
    expect_no_error(.configure_tmb_threads(1L, DLL = "rpbnb")),
    1L
  )
})
