...
test_that(".scaling_vec defaults to identity and validates its input", {
  ...
  expect_silent(rpbnb:::.scaling_vec(list(zz = c(center = 1, scale = 2)), cn))

  expect_error(rpbnb:::.scaling_vec(list(x1 = c(center = 0)), cn), "center")
  expect_error(
    rpbnb:::.scaling_vec(list(x1 = c(center = 0, scale = 0)), cn),
    "positive"
  )
  expect_error(rpbnb:::.scaling_vec(c(x1 = 1), cn), "named list")
})
...
test_that("raw_scale adds a section and is absent by default", {
  ...
  plain <- readLines(do.call(rpbnb:::.write_truck_results_markdown, args),
                     warn = FALSE)
  expect_false(any(grepl("original units", plain)))

  withr <- do.call(rpbnb:::.write_truck_results_markdown,
...
test_that(".log_vars_flag validates against the design", {
  ...
  expect_identical(rpbnb:::.log_vars_flag(NULL, cn, sel, is_bin),
                   c(FALSE, FALSE))
  expect_identical(rpbnb:::.log_vars_flag("lv", cn, sel, is_bin),
                   c(TRUE, FALSE))
  expect_error(rpbnb:::.log_vars_flag("nope", cn, sel, is_bin), "not found")
  expect_error(rpbnb:::.log_vars_flag("x2", cn, sel, is_bin), "binary")
  expect_error(rpbnb:::.log_vars_flag(1L, cn, sel, is_bin), "character")
})
