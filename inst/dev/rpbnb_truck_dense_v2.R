...
# version of inst/dev/rpbnb_truck_dense.R with the classic-engine side removed.
#
...
# inst/dev/rpbnb_truck_open_v2.R; see that script's header for the trimming
# rationale.
#
...
#     Rscript inst/dev/rpbnb_truck_dense_v2.R
# =============================================================================
...
# copula("normal") runs multithreaded by default (since 0.4.6; the SIGSEGV
# the old single-thread cap guarded against was fixed in 0.4.4).  Set
# disable_parallel_gaussian = TRUE if a Gaussian run needs to be pinned to
# one thread.
dependence <- "famoye"
...
ctrl <- rpbnb_control(
  ...
  # per-draw cost (see inst/dev/rpbnb_truck_dense.R's header), so leaving it
