...
# version of inst/dev/rpbnb_truck_open.R with the classic-engine side removed.
#
...
# and the maxLik-only control knob hess_eps is dropped since nothing here
# reads it (it only matters under se_method = "numeric").  se_method itself
# is NOT a dropped knob here: this script's own fit below runs
# engine = "classic" (rpbnb_truck_open.R's "TMB only" framing is stale on
# that point), and se_method is fit_rpbnb()'s only SE-method knob -- see the
# control object below for what it is set to and why.
#
...
#     Rscript inst/dev/rpbnb_truck_open_v2.R
# =============================================================================
...
# object.  copula("normal") runs multithreaded by default (since 0.4.6; the
# SIGSEGV the old single-thread cap guarded against was fixed in 0.4.4).  Set
# disable_parallel_gaussian = TRUE below if a Gaussian run needs to be pinned
# to one thread.
...
# ---- Control object ------------------------------------------------------------
# se_method = "opg" is pinned explicitly, not left to the default -- it's
# redundant with it right now (dependence = copula("frank") under
# engine = "classic" already defaults se_method to "opg"; see ?rpbnb_control),
# but pinning survives a later change to `dependence` above (e.g. back to
# "famoye", whose own default is "numeric") without silently changing which
# SE method this script uses. It genuinely IS read here, unlike under
# engine = "tmb" (not used by this script), which ignores se_method entirely
# and computes its own Hessian via optimHess()/sdreport() instead.
ctrl <- rpbnb_control(
  ...
  # per-draw cost (see inst/dev/rpbnb_truck_tmb.R's header), so leaving it engaged
