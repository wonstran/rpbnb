# Memory calibration harness for the rpbnb TMB engine
#
...
#
# Usage
#   Rscript inst/dev/benchmark_memory.R                     # run the grid
#   Rscript inst/dev/benchmark_memory.R --out results.csv   # choose the output file
#   Rscript inst/dev/benchmark_memory.R --child FAM N DRAWS THREADS   # one cell
#
...
measure_cell <- function(family_code, n, draws, threads) {
  ...
  started <- Sys.time()
  suppressWarnings(TMB::openmp(n = threads, DLL = "rpbnb"))
  obj <- TMB::MakeADFun(
    data = spec$data, parameters = spec$parameters, map = spec$map,
    DLL = "rpbnb", silent = TRUE
  )
...
if (!length(self)) self <- "inst/dev/benchmark_memory.R"
