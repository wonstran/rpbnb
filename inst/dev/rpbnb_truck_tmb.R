...
# data. The TMB counterpart of inst/dev/rpbnb_truck.R: same data, formulas,
...
# thread budget as the main fit. Multithreaded Gaussian-copula evaluation is
# the default since 0.4.6 (the SIGSEGV the old single-thread cap guarded
# against was fixed in 0.4.4), so no override is needed here even though
# dependence is a Gaussian copula (see below); disable_parallel_gaussian =
# TRUE would pin both the main fit and every restricted refit to one thread
# (see ?fit_rpbnb_tmb, `disable_parallel_gaussian`).
#
...
# nlminb). inst/dev/tmb_rpbnb_frank_open.R uses "laplace" instead -- see its
...
#     Rscript inst/dev/rpbnb_truck_tmb.R
# =============================================================================
...
# inst/dev/tmb_rpbnb_frank_open.R's header for the memory-vs-draws calibration
...
t_fit <- system.time(
  fit <- rpbnb(
    ...
    # disable_parallel_gaussian = TRUE,  # pins the Gaussian fit to one thread
...
# externally (Task Manager, or inst/dev/tmb_benchmark_memory.R) if a number is
# needed.
