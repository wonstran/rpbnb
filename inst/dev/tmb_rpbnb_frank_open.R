# load_all() rather than library(rpbnb) so the script runs against the working
# tree while the source is being iterated on; switch to library(rpbnb) once the
# tree is installed.
#
# The hazard this comment used to describe is worth keeping in mind: an older
# installed build predated the `scaling`/`log_vars` arguments used below, and
# because both diagnostic functions take `...`, those arguments were SILENTLY
# SWALLOWED -- the script printed standardized numbers under an "original units"
# heading and only failed later, at the exporter. Nothing in the version string
# distinguished the two builds. That class of failure is now handled
# structurally at the fitting layer: rpbnb() validates every extra argument
# against the selected engine's formals and errors on anything that does not
# belong, rather than letting it disappear into `...`. The diagnostic functions
# below still take `...`, so check their signatures if a scaling argument
...
# (REGISTER_ATOMIC in src/rpbnb_tmb.cpp): measured on this model at 8 threads,
...
# externally (Task Manager, or inst/dev/benchmark_memory.R) if a number is needed.
cat(sprintf(
...
coef_orig_units_output <- capture.output({
  cat("\n--- Equation 1 (y1) ---\n")
  rpbnb:::.print_tbl(coef_orig_units(1L))
  cat("\n--- Equation 2 (y2) ---\n")
  rpbnb:::.print_tbl(coef_orig_units(2L))
