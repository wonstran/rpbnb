...
#      engines.
#
# Relationship to the neighbouring scripts: inst/dev/rpbnb_truck.R is the
# classic-engine-only version of this fit and inst/dev/rpbnb_truck_tmb.R the
...
#     Rscript inst/dev/rpbnb_truck_open.R
# =============================================================================
...
# object.  On the TMB engine, copula("normal") runs multithreaded by default
# (since 0.4.6; the SIGSEGV the old single-thread cap guarded against was
# fixed in 0.4.4) -- disable_parallel_gaussian = TRUE pins it to one thread
# if needed.  The classic engine has no such cap or knob at all.
...
for (eng in engines) {
  ...
  # method / disable_parallel_gaussian / boundary_draws are TMB-only.  rpbnb()
