...
fit_bnbr_maxlik_df <- function(data, f1, f2,
  ...
  # capture LL values at each evaluation (sum-scale, since maxLik uses a scalar)
  ll_trace <- numeric(0)
  ll_fun <- function(p) {
  ...
  # gradient switch: analytic vs numeric (numDeriv)
...
# Toggle analytic gradient here:
fit <- fit_bnbr_maxlik_df(
  data = df,
  f1   = f1_fit,   # NB for Y1
  f2   = f2_fit,   # NB for Y2
  start = NULL, method = "BFGS",
  use_analytic_grad = TRUE   # <- set FALSE to use numeric gradient
)
