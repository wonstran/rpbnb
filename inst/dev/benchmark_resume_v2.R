...
for (i in pending_idx) {
  ...
  # Multithreaded Gaussian-copula evaluation is the default since 0.4.6 (the
  # SIGSEGV it used to guard against was fixed in 0.4.4), so no override is
  # needed here for any family.
  cat("\n", paste(rep("=", 72), collapse=""), "\n", sep="")
  cat(sprintf("[%d/%d] RESUME %s  (dependence=%s, method=%s)\n",
              i, length(results), label, dep_fam, tmb_method))
