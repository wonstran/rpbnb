...
data$cr = data$ALL_3*1000000/(365*data$aadt*0.1)
...
write.csv(sorted, "C:\\Users\\litabook\\repos\\truck\\results\\smoothing_dense_results_all.csv", row.names = FALSE)
...
ggplot(sorted, aes(x=SR40_MI3)) +
  ...
  geom_vline(aes(xintercept = 55), color = "green4", linetype = "dashed", size=1) +
  geom_label(aes(x=36, y = 14, label="Problematic IL: 36"), color="red") +
  geom_label(aes(x=55, y = 18, label="Preferred IL: 55"), color="green4") +
  geom_label(aes(x=46, y = 3, label="Histogram of Friction"), color="gray28") +
  theme_bw() + 
  ...
  ggtitle("Friciton Investigory Levels for All Crashes", 
...
ggsave('C:\\Users\\litabook\\repos\\truck\\pics\\il_dense_seg_paper_all.png', width = 7, height = 6, device='png', dpi=300)
