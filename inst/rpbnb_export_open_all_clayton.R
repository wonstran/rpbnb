#!/usr/bin/env Rscript
# =============================================================================
# Fit RP-BNB model (non-TMB) to export_open_all.csv with Clayton copula
# Uses BFGS optimization with random coefficients
# =============================================================================

library(rpbnb)

# ---- Load data ---------------------------------------------------------------
data <- read.csv(system.file(
  "extdata", "export_open_all.csv",
  package = "rpbnb", mustWork = TRUE
))

cat("=== Fitting RP-BNB (non-TMB, Clayton Copula) to export_open_all.csv ===\n")
cat("Observations :", nrow(data), "\n")
cat("Columns     :", paste(names(data), collapse = ", "), "\n\n")

# ---- Model specification ----------------------------------------------------
numeric_cols <- names(data)[sapply(data, is.numeric) & names(data) != "y1" & names(data) != "y2"]
rhs <- as.formula(paste("~", paste(numeric_cols[1:min(5, length(numeric_cols))], collapse = " + ")))

f1 <- update(rhs, y1 ~ .)
f2 <- update(rhs, y2 ~ .)

random_var <- numeric_cols[1]

cat("Formula 1:", deparse(f1), "\n")
cat("Formula 2:", deparse(f2), "\n")
cat("Random on:", random_var, "\n\n")

# ---- Fit model ---------------------------------------------------------------
t_fit <- system.time(
  fit <- fit_rpbnb(
    formula_1 = f1,
    formula_2 = f2,
    data      = data,
    random_1  = random_var,
    random_2  = random_var,
    dependence = copula("clayton"),
    draws     = 250,
    seed      = 20260809,
    control   = rpbnb_control(
      n_cores = parallel::detectCores() - 2,
      compute_se = TRUE,
      print_level = 1
    )
  )
)[["elapsed"]]

cat(sprintf("\nEstimation finished in %.1f s\n\n", t_fit))

# ---- Model summary ----------------------------------------------------------
print(fit)

cat("\nlogLik =", round(fit$logLik, 2),
    "  AIC =", round(fit$AIC, 2),
    "  BIC =", round(fit$BIC, 2),
    "  npar =", fit$npar, "\n")

cat("\nCopula family:", fit$cop_family, "\n")
cat("Copula parameter:", round(fit$cop_par, 4), "\n")
cat("Copula tau (rank correlation):", round(fit$cop_tau, 4), "\n")
