...
[**Reference manual (PDF)**](ref/rpbnb_0.4.8.pdf) — full CRAN-style function
...
download the file first (substitute the version you want for `0.4.8`; browse
all releases at
...
```r
install.packages(
  "https://github.com/wonstran/rpbnb/releases/download/v0.4.8/rpbnb_0.4.8.zip",
...
```r
install.packages(
  "https://github.com/wonstran/rpbnb/releases/download/v0.4.8/rpbnb_0.4.8-macos-arm64.tgz",
  repos = NULL)
```
...
```r
install.packages(
  "https://github.com/wonstran/rpbnb/releases/download/v0.4.8/rpbnb_0.4.8.tar.gz",
...
formulas, dependence choice, diagnostics), not just a snippet.

```r
library(rpbnb)

# List every example script actually installed on your machine
list.files(system.file(package = "rpbnb"), pattern = "^example_.*\\.R$")

# Run one from inside an R session...
source(system.file("example_bnb.R", package = "rpbnb"))
```

...or from a shell, without starting an R session first:

```bash
Rscript -e 'source(system.file("example_bnb.R", package = "rpbnb"))'
```

From a source checkout (not yet installed), run the file in `inst/` directly:
...
deliberately against the memory you actually have — `rpbnb_tmb_max_workload()`
converts a memory budget into the right value — and `max_workload = Inf`
