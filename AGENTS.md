...
- `inst/dev/legacy/` — original reference scripts (preserved for comparison, not used by the package)
- `inst/dev/` — investigation, benchmark, and truck/export scripts not meant as
  user-facing examples (moved out of `inst/`'s top level so `inst/example_bnb.R`
  and friends stay the only things a new user sees there)
- `NEWS.md` — changelog
