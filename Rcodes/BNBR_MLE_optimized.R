# =========================================================
# Famoye BNBR with maxLik (BFGS) — Optimized
#  * Analytic gradient (vector) + numeric Hessian
#  * Vectorized, NA-safe, robust summary with significance stars
#  * Performance tweaks:
#      - digamma() instead of loops for Σ 1/(r+j)
#      - no for-loops over columns (row-scaling trick)
#      - cache common terms, fewer repeated ops
# =========================================================
...
# Vectorized ∂c/∂β for a whole design matrix X:
...
# ---- Per-observation analytic SCORE matrix (n x k), optimized ----
bnb_score_mat <- function(par, y1, y2, X1, X2) {
  ...
  # d c / d m and d c / d beta (matrix form; no loops over columns)
  ...
  # ∂ℓ/∂β blocks (row scaling; no for loops)
  # score_beta = X * w  - (∂c/∂β) * pen
  score_beta1_mat <- sweep(X1, 1, w1, `*`) - sweep(dc1_dbetas, 1, pen1, `*`)
  score_beta2_mat <- sweep(X2, 1, w2, `*`) - sweep(dc2_dbetas, 1, pen2, `*`)
  
  # helpers for m1,m2 using digamma identity: sum_{j=0}^{y-1} 1/(r+j) = ψ(r+y) - ψ(r)
  r1 <- 1/m1; r2 <- 1/m2
...
# install.packages("maxLik"); install.packages("numDeriv")
library(maxLik)
...
fit <- maxLik(
  ...
  start   = par0,
  method  = "BFGS",
  control = list(printLevel = 2, tol = 1e-8, reltol = 1e-10)
)
