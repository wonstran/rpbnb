...
#' \code{FRANK_THETA_MAX} in \code{src/rpbnb_tmb.cpp}.  It caps attainable
...
#'
#' The template maps the working parameter through
#' \code{theta = exp(clamp(z_dep, -20, 20))} (\code{src/rpbnb_tmb.cpp}, the
...
#'   * `dev-docs/tmb-reference/rpbnb.tmb-reference-manual.html` -- checked by
...
#' @param gradtol Stationarity tolerance for the score, applied \emph{relative}
...
rpbnb_tmb_control <- function(iterlim = 500L,
  ...
  if (length(gradtol) != 1L || !is.numeric(gradtol) || is.na(gradtol) ||
      !is.finite(gradtol) || gradtol <= 0) {
    stop("gradtol must be one positive finite number.", call. = FALSE)
  }
  if (length(restarts) != 1L || !is.numeric(restarts) || is.na(restarts) ||
      !is.finite(restarts) || restarts < 0 || restarts != floor(restarts) ||
      restarts > .Machine$integer.max) {
