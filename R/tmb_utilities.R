...
.calibration_doc <- function(calibration = TAPE_CALIBRATION) {
  ...
  paste0(
    ...
    "behaviour.\n\n",
    "With \\code{parallel_tape = FALSE} the budget is per fit; with the ",
    "default \\code{parallel_tape = TRUE} the tapes are built concurrently ",
    "and the guard multiplies the workload by the realized thread count ",
    "(see \\code{n_cores} above for how that count is realized).\n\n",
