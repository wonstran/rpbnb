...
fit_rpbnb_tmb <- function(formula_1, formula_2, data,
  ...
  # Prepare data. This is the shared .prepare_bnb_data() (R/data_prep.R), also
  # used by the Rcpp engine; it returns terms/xlevels/contrasts per equation
  # rather than the pre-assembled `model_meta` this fitter used to get, so
  # model_meta is rebuilt below in exactly its previous shape -- predict() and
  # every already-saved rpbnb_tmb_fit .rds depend on that layout.
  prep <- .prepare_bnb_data(formula_1, formula_2, data)
  Y1 <- prep$Y1; Y2 <- prep$Y2
  X1 <- prep$X1; X2 <- prep$X2
  n <- prep$n

  # The shared data prep understands offset() terms; the TMB template does not
  # (its linear predictor is X %*% beta plus random deviations, with no offset).
  # Erroring is mandatory: silently dropping the offset would produce a fit that
  # looks fine and is wrong.
  if (any(prep$off1 != 0) || any(prep$off2 != 0)) {
    stop("The TMB engine does not support offset() terms in a formula. ",
         "Use fit_rpbnb() (engine = \"cpp\"), which does, or fold the offset ",
         "into the response scale before fitting.", call. = FALSE)
  }

  model_meta <- list(
    eq1 = list(terms     = stats::delete.response(prep$terms1),
               xlevels   = prep$xlevels1,
               contrasts = prep$contrasts1),
    eq2 = list(terms     = stats::delete.response(prep$terms2),
               xlevels   = prep$xlevels2,
               contrasts = prep$contrasts2)
  )
  ...
  set.seed(seed)
  if (total_rand > 0) {
    Z <- .tmb_halton_uniform(draws, total_rand, burn = control$halton_burn)
